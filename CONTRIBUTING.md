...
Локально экспортировать его не нужно: `bootstrap/config/application.yml` подставляет заведомо
локальный ключ, и Spring Boot читает этот файл из рабочего каталога сам — одинаково и для
`./gradlew :bootstrap:bootRun`, и для Spring Boot run configuration в IDE, которая стартует
main-класс напрямую, минуя Gradle (рабочий каталог у неё — каталог модуля `bootstrap`; если в
конфигурации запуска стоит корень репозитория, поменяйте на `$MODULE_WORKING_DIR$`). Шифрование
при этом включено, а экспортированный `CONTENT_ENCRYPTION_KEY` побеждает подставленный.

В поставке этого файла нет: `bootstrap/config` — не каталог ресурсов, поэтому он не попадает
в jar, а Dockerfile копирует из модуля только `bootstrap/src`. Всё, что похоже на развёртывание —
`java -jar` и контейнер, — ключ требует по-прежнему и без него не стартует.
