...
Исключение ровно одно — задача `bootRun`: она подставляет заведомо локальный ключ, если переменной
в окружении нет, потому что отказ старта в новом терминале не сообщает разработчику ничего полезного.
Шифрование при этом включено; экспортированный ключ побеждает подставленный. Запуск из jar и из
образа — то есть всё, что похоже на развёртывание, — ключ требует по-прежнему.
