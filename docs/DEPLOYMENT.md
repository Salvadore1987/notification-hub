...
без него контекст не поднимется, тихого отката на хранение открытым текстом нет. **`bootRun`
подставляет локальный ключ сам** (см. `bootstrap/build.gradle.kts`) — шифрование при этом включено,
просто ключ известный и годится только для одноразовой локальной базы. Запуск из jar и из образа
ключ по-прежнему требует, поэтому проверять деплой нужно так:
