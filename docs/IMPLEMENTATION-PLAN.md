...
> `SecretResolverPort`, после Phase 8 — `ProviderStatsPort`; контекст всё ещё не стартует целиком — ждут своих фаз
> `FrequencyCounterPort` (Phase 10), `MetricsPort` (Phase 13), `KillSwitchPort` (Phase 14), `CustomerPreferencePort`
> (фаза 2 по SRS) и `ProviderProbePort` (задел, для SMS реализации не будет).
