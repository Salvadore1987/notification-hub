...
> `CustomerPreferencePort` (заглушкой, как предписывает FR-8.2); контекст всё ещё не стартует целиком — ждут своих
> фаз `MetricsPort` (Phase 13), `KillSwitchPort` (Phase 14) и `ProviderProbePort` (задел, для SMS реализации не будет).
