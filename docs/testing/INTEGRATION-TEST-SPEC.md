...
**Стадии конвейера упорядочены, и это правило написания кейса.** Ожидаемая причина отказа в кейсе —
это причина **первой** стадии, которая данный вход отвергает. Вход, отвергнутый ранней стадией, до
поздней не доходит вовсе, поэтому кейс, написанный от ветки поздней стадии, проверяет недостижимое
состояние и покраснеет навсегда, что бы Модуль ни делал. Проверять это приходится руками: области
набора нарезаны по слоям (таблица ниже), а слой о соседях не знает — так и появился дефект **D-9**
(`IT-RTE-007` написан от ветки `Router`, которую `MessageValidator` перехватывает стадией раньше;
кейс снят, разбор — [`runs/2026-08-13-stage-5.md`](runs/2026-08-13-stage-5.md) §3).

**Умолчания тоже упорядочены, и это второе правило написания кейса.** Проверка вида «значение
выведено из X» обязана сначала снять всё, что стоит **выше X** в цепочке умолчаний, — иначе она
проверяет не Модуль, а конфигурацию контура. Пример, на котором это нашлось: приоритет сообщения
выбирается как «явный из документа → `stream.default_priority` → умолчание класса трафика», и
умолчание потока несут все одиннадцать потоков эталонного контура, поэтому кейс «приоритет выведен
из класса» на нём недостижим — а на трёх потоках, где умолчание совпадает с классовым, он зеленеет,
ничего не проверив. Оба исхода одинаково бесполезны, и разводятся они не переписыванием проверки,
а `@arrange`, который снимает верхнюю ступень (дефект **D-31**, разбор —
[`runs/2026-08-15-stage-11.md`](runs/2026-08-15-stage-11.md) §3.1).

**И третье, про формулировку самой проверки: спрашивать факт, а не момент времени.** Проверка,
которая останется зелёной на полностью сломанном Модуле, — не проверка. Полезно прямо спросить себя:
«что должно произойти, чтобы эта строка стала `f`?» — и если ответ «почти ничего», переписать. Так
дважды подряд: `min(OTP) < max(NOTIFICATION)` истинно и когда OTP пятисотый из пятисот одного
(**D-32**), а `status IN ('ACCEPTED','PROCESSING')` истинно ровно те секунды, пока рассылка не
закрылась сама (**D-25**, [`runs/2026-08-15-stage-10.md`](runs/2026-08-15-stage-10.md) §3.1).
...
|---|---|---|---|
| `SMS` | `PRIMARY_ONLY` | `[MOCK_PRIMARY, MOCK_RESERVE]` | базовый канал большинства кейсов |
| `EMAIL` | `PRIMARY_ONLY` | `[MOCK_EMAIL]` | |
| `PUSH` | `PRIMARY_ONLY` | `[MOCK_PUSH]` | терминальный статус `SENT_TO_PROVIDER` (PU-12) |
...
**Коды провайдеров пишутся через подчёркивание** (`MOCK_PRIMARY`, а не `MOCK-PRIMARY`): `ProviderCode` — это `^[A-Z0-9][A-Z0-9_]{1,31}$`, и тот же шаблон стоит в контракте админ-BFF. Дефис база примет — она проверяет только длину, — но собранная из такой строки конфигурация ломается на первом же чтении: маршрутизация рехидратирует профиль через доменный объект, и **любая** отправка отвечает `400 VALIDATION_FAILED` с текстом `ProviderCode.value has an invalid format`. Тип адаптера — наоборот, строчными и с дефисом (`^[a-z0-9][a-z0-9-]{1,31}$`); это разные словари, и путать их нельзя.
...
```bash
sql/testing/bin/run-case.sh --list          # все 156 идентификаторов
...
Порядок ручного прогона — в [`TEST-PLAN.md`](TEST-PLAN.md): этапы, профили запуска (четырнадцать кейсов требуют не другого состояния БД, а других свойств приложения) и чекбоксы на все 156 кейсов. Порядком кейсы друг от друга не связаны — см. ниже; план экономит перезапуски и время на разбор, а не создаёт зависимости.
