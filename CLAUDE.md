...
**Phase 1–3 done (scaffolding + domain model + application ports/use cases), Phase 4 next (PostgreSQL persistence).** `domain/` holds the full model (`model/{vo,type,content}`, aggregates, `service/{SegmentCalculator,Router,FallbackChain}`, `support/{Guard,UuidV7}`) with 268 unit tests at 97% line coverage; `application/` holds `port/in` + `port/in/command`, `port/out` (+`port/out/provider`), `service` (use cases) with `service/{pipeline,support}`, `dto/`, `mapper/`, `policy/`, 59 unit tests at 86%; `adapter/` still holds only `package-info.java`.
