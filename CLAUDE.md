...
**Phase 1–8 done (scaffolding + domain model + application ports/use cases + PostgreSQL persistence + transactional outbox relay → Kafka + inbound REST/Kafka/callback adapters + SMS provider adapters + routing/channel/provider/quota configuration in the DB), Phase 9 next (templates and personalisation).** `domain/` holds the full model (`model/{vo,type,content}`, aggregates, `service/{SegmentCalculator,Router,FallbackChain}`, `support/{Guard,UuidV7}`) with 281 unit tests at ~97% line coverage; `application/` holds `port/in` + `port/in/{command,query}`, `port/out` (+`port/out/provider`), `service` (use cases) with `service/{pipeline,support}`, `dto/`, `mapper/`, `policy/`, 95 unit tests; `adapter/` holds `out/persistence` (`config`, `messaging`, `delivery`, `template`, `audit`, `json`, `crypto`, `support`), `out/kafka`, `out/time`, `out/secret`, `out/provider/{support,playmobile,smsgate}`, `in/{contract,rest,kafka,callback,scheduler}`; 218 unit tests plus eight Testcontainers suites and two WireMock contract suites, all tagged `integration`.
