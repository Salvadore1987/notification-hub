...
**Phase 1–9 done (scaffolding + domain model + application ports/use cases + PostgreSQL persistence + transactional outbox relay → Kafka + inbound REST/Kafka/callback adapters + SMS provider adapters + routing/channel/provider/quota configuration in the DB + templates and personalisation), Phase 10 next (filtering and compliance).** `domain/` holds the full model (`model/{vo,type,content}`, aggregates, `service/{SegmentCalculator,Router,FallbackChain}`, `support/{Guard,UuidV7}`) with 288 unit tests at ~97% line coverage; `application/` holds `port/in` + `port/in/{command,query}`, `port/out` (+`port/out/provider`), `service` (use cases) with `service/{pipeline,support}`, `dto/`, `mapper/`, `policy/`, 113 unit tests; `adapter/` holds `out/persistence` (`config`, `messaging`, `delivery`, `template`, `audit`, `json`, `crypto`, `support`), `out/kafka`, `out/time`, `out/secret`, `out/provider/{support,playmobile,smsgate}`, `in/{contract,rest,kafka,callback,importer,scheduler}`; 224 unit tests plus nine Testcontainers suites and two WireMock contract suites, all tagged `integration`.
