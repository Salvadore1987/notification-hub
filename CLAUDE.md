...
**Phase 1–6 done (scaffolding + domain model + application ports/use cases + PostgreSQL persistence + transactional outbox relay → Kafka + inbound REST/Kafka/callback adapters), Phase 7 next (SMS provider adapters).** `domain/` holds the full model (`model/{vo,type,content}`, aggregates, `service/{SegmentCalculator,Router,FallbackChain}`, `support/{Guard,UuidV7}`) with 276 unit tests at 97% line coverage; `application/` holds `port/in` + `port/in/{command,query}`, `port/out` (+`port/out/provider`), `service` (use cases) with `service/{pipeline,support}`, `dto/`, `mapper/`, `policy/`, 64 unit tests at 86%; `adapter/` holds `out/persistence` (`config`, `messaging`, `delivery`, `template`, `audit`, `json`, `crypto`, `support`), `out/kafka`, `out/time`, `in/{contract,rest,kafka,callback,scheduler}`; 97 unit tests plus seven Testcontainers suites tagged `integration`.
