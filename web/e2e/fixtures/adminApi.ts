 ...
 *
 * `page` подменяется здесь же, и это не удобство, а необходимость: панель за SSO (ADR-0037), а
 * сессии у новой вкладки нет — первый переход упирается в форму входа (ADR-0043). Вход и ожидание
 * экрана живут в фикстуре, а не в каждом spec'е, потому что забыть их — значит получить плавающий
 * тест, а не ошибку: `page.goto` возвращает управление по загрузке документа, то есть до того, как
 * панель вообще что-то отрисовала, и тест, смотрящий на страницу сразу (axe, например, а не
 * локатор с автоожиданием), увидел бы форму.
 */
...
/**
 * Панель отрисовалась — и отрисовала **запрошенный экран**, а не дашборд. С формой входа внутри
 * панели адрес никуда не уходит, и это как раз то, что должно оставаться правдой: глубокая ссылка
 * переживает вход. Проверяется здесь, а значит каждым сценарием, а не отдельным тестом.
 */
